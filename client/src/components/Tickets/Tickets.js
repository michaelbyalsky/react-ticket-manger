...
const Tickets = ({ hideTicket, ticketsData, restoreTicket, doneTicket }) => {
  ...
  const stringToTime = (date) => {
    ...
    const getTime = `${dd.getFullYear().toString()}-${`0${(
      dd.getMonth() + 1
    ).toString()}`.slice(-2)}-${`0${dd.getDate().toString()}`.slice(
      -2
  ...
  const handleChange = (panel) => (event, isExpanded) => {
    console.log(event.target.childNodes[0].outerHTML);
            ...
            <Grid container alignItems="center">
              <Grid item xs>
                <Typography gutterBottom>
                  {`by ${ticket.userEmail} | ${stringToTime(
                    ticket.creationTime
                  )}`}
                </Typography>
              </Grid>
