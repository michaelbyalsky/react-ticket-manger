...
//   });
  ...
  test("can update ticket status to done", async () => {
    const filterText = "h";
    const getFilteredTicketsMock = await nock("http://localhost:3000/", {
      allowUnmocked: true,
    })
      .post(`/api/tickets?${mockData[0].id}/done`)
    //   .query(({ searchText }) => {
    ...
    const elements = await page.$$(".ticket");
    await page.click(".ticket .doneButton");
    await new Promise((resolve) => setTimeout(resolve, 3000));
    expect(getFilteredTicketsMock.isDone()).toBe(true);
    const elements2 = await page.$$(".ticket .unDoneButton");
